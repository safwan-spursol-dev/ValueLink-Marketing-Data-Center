...
try {
    ...
    // ✨ MAPPING (job_title added here)
    $client_name    = $data['name'] ?? 'Unknown';
    $email          = $data['email'] ?? null;
    $phone          = $data['phone'] ?? null;
    $company        = $data['company'] ?? null;
    $job_title      = $data['job_title'] ?? null; // ✨ Naya Field
    ...
    $sales_team = $teamStmt->fetchAll(PDO::FETCH_COLUMN); 
    ...
    // ✨ 7. INSERT QUERY (job_title column aur variable add kar diya)
    $sql = "INSERT INTO leads 
            (client_name, email, phone, company, job_title, message, user_role, order_volume, preferred_date, event_title, source, form_name, submission_url, status, assigned_to, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'new', ?, NOW())";
            
    $stmt = $pdo->prepare($sql);
    
    // ✨ EXECUTE (job_title array mein pass kar diya)
    if($stmt->execute([
        $client_name, $email, $phone, $company, $job_title, $message, 
