...
// =========================================================
// 1. BACKEND HANDLERS (DELETE, AJAX STATUS, NOTES, QUALIFICATION)
// =========================================================
...
// E. AJAX: UPDATE QUALIFICATION SCALE
...
<style>
    ...
    /* ✨ NEW: Status Dropdown behaving like a Badge */
    .status-badge-select {
                ...
                <div class="col-lg-12 mb-4">
                    <div class="card mb-5">
                        ...
                        <div class="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
                            <h5 class="card-title mb-0" style="min-width: 150px;">Received Leads <br><span id="total-leads-badge" class="badge bg-secondary-subtle text-secondary mt-1"><?php echo $total_leads; ?> Total</span></h5>
                            
                            <form id="filter-form" method="GET" action="leads-list.php" class="d-flex flex-wrap align-items-center gap-2 w-100 justify-content-md-end">
                                <input type="hidden" name="limit" value="<?php echo $limit; ?>">
                                
                                <div class="input-group input-group-sm" style="flex: 1; min-width: 180px; max-width: 220px;">
                                ...
                                <select class="form-select form-select-sm" name="form_filter" style="flex: 1; min-width: 140px; max-width: 200px;">
                                    <option value="">All Forms</option>
                                    <?php foreach ($formNames as $name): ?>
                                        <option value="<?php echo htmlspecialchars($name); ?>" <?php echo ($selected_form === $name) ? 'selected' : ''; ?>><?php echo htmlspecialchars($name); ?></option>
                                    <?php endforeach; ?>
                                </select>

                                <select class="form-select form-select-sm" name="qualification_filter" style="flex: 1; min-width: 130px; max-width: 160px;">
                                ...
                                <div class="d-flex gap-2">
                                    <button type="button" id="export-btn" class="btn btn-sm btn-success w-100 w-sm-auto" title="Export to Excel">
                                        <i class="ri-file-excel-2-fill align-bottom me-1"></i> Export
                                    </button>

                                    <span id="filter-reset-btn">
                                        <?php if(!empty($selected_form) || !empty($date_range) || !empty($selected_qual) || !empty($selected_status)): ?>
                                            <a href="leads-list.php" class="btn btn-sm btn-soft-danger w-100 w-sm-auto" title="Clear Filters"><i class="ri-close-line"></i></a>
                                        <?php endif; ?>
                                    </span>
                                </div>
                            </form>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php if(!empty($lead['event_title'])): ?>
                                                                <div class="info-row">
                                                                    ...
                                                                    <span class="val"><?php echo htmlspecialchars($lead['event_title']); ?></span>
                                                                </div>
                                                            <?php endif; ?>
                                                            ...
                                                            <?php if(!empty($lead['job_title'])): ?>
                                                                <div class="info-row">
                                                                    <span class="lbl">Job Title:</span>
                                                                    <span class="val"><?php echo htmlspecialchars($lead['job_title']); ?></span>
                                                                </div>
                                                            <?php endif; ?>
                                                        </td>
                                                        ...
                                                        <td>
                                                            <?php 
                                                                ...
                                                                elseif ($st == 'in_progress') { $cls = 'warning'; $st = 'In Progress'; }
                                                                elseif ($st == 'converted') { $cls = 'success'; $st = 'Converted'; }
                                                                elseif ($st == 'lost') { $cls = 'danger'; $st = 'Lost'; }
                                                            ?>
                                        ...
                                        <?php if ($total_pages > 1): ?>
                                            <nav aria-label="Page navigation">
                                                <ul class="pagination pagination-sm mb-0 justify-content-center">
                                                    ...
                                                    <?php 
                                                    // URL params builder for pagination links
                                                    $params = "&limit=$limit&form_filter=".urlencode($selected_form)."&date_range=".urlencode($date_range)."&qualification_filter=".urlencode($selected_qual)."&status_filter=".urlencode($selected_status); 
                                                    ?>

                                                    <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                                                        <a class="page-link" href="?page=<?php echo $page - 1; ?><?php echo $params; ?>" aria-label="Previous"><i class="ri-arrow-left-s-line"></i></a>
                                                    </li>
                                                    ...
                                                    <li class="page-item <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>">
                                                        <a class="page-link" href="?page=<?php echo $page + 1; ?><?php echo $params; ?>" aria-label="Next"><i class="ri-arrow-right-s-line"></i></a>
                                                    </li>
                                                </ul>
                                            </nav>
                                        <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (wrapper) {
        ...
        // ✨ STATUS & QUALIFICATION DROPDOWN AJAX LISTENER ✨
        wrapper.addEventListener("change", function(e) {
            
            // 1. Status Dropdown Logic
            if (e.target.classList.contains('update-status-dropdown')) {
                ...
                fetch('leads-list.php', { method: 'POST', body: formData })
                    .then(res => res.json())
                    .then(data => {
                        if(data.success) { location.reload(); } // Reload taaky naya color/badge load ho
                        else { alert('Error updating status: ' + data.error); e.target.style.opacity = '1'; }
                    })
                    .catch(err => {
                        console.error(err); e.target.style.opacity = '1'; alert('Something went wrong!');
                    });
            }

            // 2. Qualification Dropdown Logic
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', function() {
            // Get current filter values from URL or Form inputs
            const formFilter = document.querySelector('select[name="form_filter"]').value;
            const dateRange = document.querySelector('input[name="date_range"]').value;
            const qualFilter = document.querySelector('select[name="qualification_filter"]').value;
