...
if (isset($_POST['import_btn'])) {
    if ($_FILES['csv_file']['error'] == 0) {
        ...
        if (strpos($filename_lower, 'contact') === false) {
        ...
        elseif ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, email, phone, company, message, form_name, source, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'CSV Import', 'new', ?)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([$client_name, $email, $phone, $company, $msg_text, $form_name, $created_at]);
                        $count++;
                    } catch (PDOException $e) { }
                }
                $message = "Success! Form: <b>$form_name</b>. Imported <b>$count</b> leads.";
                $msgType = "success";
            } else { $message = "File is empty."; $msgType = "danger"; }
