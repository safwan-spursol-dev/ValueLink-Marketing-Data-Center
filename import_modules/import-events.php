...
if (isset($_POST['import_btn'])) {
    if ($_FILES['csv_file']['error'] == 0) {
        ...
        if (strpos($filename_lower, 'event') === false) {
        ...
        elseif ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                $map = ['name'=>-1, 'email'=>-1, 'company'=>-1, 'message'=>-1, 'date'=>-1, 'event_title'=>-1];
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, email, company, message, event_title, form_name, source, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'CSV Import', 'new', ?)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([$client_name, $email, $company, $msg_text, $event_title, $form_name, $created_at]);
                        $count++;
