<?php
// import-cs-anniemac.php
require_once '../config/db.php'; 
require_once '../includes/session_check.php';

$message = "";
$msgType = "";
$form_name = "Case Study Form - ValueLink Partners with AnnieMac";
$headerRowIndex = 0;
        ...
        if ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, email, company, preferred_date, message, form_name, source, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'CSV Import', 'new', ?)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([$client_name, $email, $company, $preferred_date, $msg_text, $form_name, $created_at]);
                        $count++;
