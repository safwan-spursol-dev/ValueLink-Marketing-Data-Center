...
if (isset($_POST['import_btn'])) {
    if ($_FILES['csv_file']['error'] == 0) {
        ...
        if (strpos($filename_lower, 'mobile') === false) {
        ...
        elseif ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                $map = ['name'=>-1, 'email'=>-1, 'phone'=>-1, 'company'=>-1, 'message'=>-1, 'date'=>-1, 'user_role'=>-1, 'order_volume'=>-1];
                ...
                if (isset($fileRows[$headerRowIndex])) {
                    foreach ($fileRows[$headerRowIndex] as $index => $colName) {
                        ...
                        elseif ($col === 'date' && $map['date'] === -1) { $map['date'] = $index; }
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, email, phone, company, message, user_role, order_volume, form_name, source, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'CSV Import', 'new', ?)";
