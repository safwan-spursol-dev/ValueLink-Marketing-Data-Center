<?php
// import-crosscheck-demo.php
require_once '../config/db.php'; 
require_once '../includes/session_check.php';

$message = "";
$msgType = "";
$form_name = "CrossCheck Demo Request";
$headerRowIndex = 0;
        ...
        if ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, company, email, phone, user_role, order_volume, message, preferred_date, form_name, source, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'CSV Import', 'new', ?)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([$client_name, $company, $email, $phone, $user_role, $order_volume, $msg_text, $preferred_date, $form_name, $created_at]);
                        $count++;
