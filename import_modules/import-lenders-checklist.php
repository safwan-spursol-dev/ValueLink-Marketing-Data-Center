...
$message = ""; $msgType = "";
$form_name = "Lender's Checklist Download Form";
$headerRowIndex = 0;
...
if (isset($_POST['import_btn'])) {
    if ($_FILES['csv_file']['error'] == 0) {
        ...
        if (strpos($filename_lower, 'lender') === false && strpos($filename_lower, 'checklist') === false) {
        ...
        elseif ($ext === 'csv') {
            ...
            if (count($fileRows) > 0) {
                ...
                for ($i = $headerRowIndex + 1; $i < count($fileRows); $i++) {
                    ...
                    try {
                        $sql = "INSERT INTO leads (client_name, email, form_name, source, status, created_at) VALUES (?, ?, ?, 'CSV Import', 'new', ?)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([$client_name, $email, $form_name, $created_at]);
                        $count++;
