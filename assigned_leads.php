...
$page_title = "My Assigned Leads"; // ✨ Title set for this page

// =========================================================
// 2. BACKEND HANDLERS (Same logic, but keeps user on this page)
// =========================================================
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    if ($user_role === 'admin') {
        ...
        // 👇 Redirect back to assigned_leads.php
        header("Location: assigned_leads.php?msg=deleted"); 
        exit;
    } else {
        die("Access Denied");
    }
...
// --- A. Live API Call (Same as before) ---
$wp_api_url = "https://www.valuelinksoftware.com/wp-json/vmdc/v1/get-forms"; 
...
if ($http_code == 200 && $response) {
    ...
    if (is_array($data)) { foreach ($data as $form) { $formNames[] = $form['title']; } }
} else {
...
// 🔒 CRITICAL LOGIC: FORCE FILTER BY ASSIGNED USER 🔒
$where_clauses[] = "leads.assigned_to = ?";
$params[] = $user_id;
...
<style>
    ...
    /* Pagination Styling */
    .pagination .page-item .page-link { margin: 0 5px; border-radius: 50% !important; width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; border: 1px solid #e9ebec; color: #405189; font-weight: 600; font-size: 15px; transition: all 0.3s ease; }
    .pagination .page-item .page-link i { font-size: 20px; line-height: 0; }
    .pagination .page-item .page-link:hover { background-color: #c7c7c7ff; border-color: #d8dbe0; }
    .pagination .page-item.active .page-link { background-color: #405189; border-color: #405189; color: #fff; box-shadow: 0 4px 8px rgba(64, 81, 137, 0.3); }
    .pagination .page-item.disabled .page-link { background-color: #f8f9fa; color: #adb5bd; opacity: 0.6; }
</style>
                    ...
                    <div class="card mb-5">
                        ...
                        <div class="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
                            ...
                            <h5 class="card-title mb-0">
                                <?php echo $page_title; ?> 
                                <span id="total-leads-badge" class="badge bg-secondary-subtle text-secondary"><?php echo $total_leads; ?> Total</span>
                            </h5>

                            <form id="filter-form" method="GET" action="assigned_leads.php" class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-2 w-100 w-md-auto">
                                ...
                                <div class="input-group input-group-sm" style="flex: 1; min-width: 220px;">
                                    ...
                                    <input type="text" class="form-control bg-light border-light" name="date_range" id="date_range_picker" placeholder="Select Date Range" value="<?php echo htmlspecialchars($date_range); ?>">
                                </div>
                                ...
                                <select class="form-select form-select-sm" name="form_filter" style="flex: 1; min-width: 180px;">
                                    ...
                                    <?php foreach ($formNames as $name): ?>
                                        <option value="<?php echo htmlspecialchars($name); ?>" <?php echo ($selected_form === $name) ? 'selected' : ''; ?>>
                                ...
                                <span id="filter-reset-btn">
                                    <?php if(!empty($selected_form) || !empty($date_range)): ?>
                                        <a href="assigned_leads.php" class="btn btn-sm btn-soft-danger w-100 w-sm-auto" title="Clear Filters"><i class="ri-close-line"></i></a>
                                    <?php endif; ?>
                                </span>
                            </form>
                        </div>
                                            ...
                                            <?php if (count($leads) > 0): ?>
                                                <?php foreach ($leads as $lead): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php if(!empty($lead['event_title'])): ?><div class="info-row"><span class="lbl">Event:</span> <span class="val"><?php echo htmlspecialchars($lead['event_title']); ?></span></div><?php endif; ?>
                                                            ...
                                                            <div class="val" style="font-size: 13px; line-height: 1.4;">
                                                                ...
                                                                if (strlen($msgText) > 80) { echo substr($msgText, 0, 80) . '... <a href="#" data-bs-toggle="modal" data-bs-target="#viewMsgModal" data-msg="'.$msgText.'">Read More</a>'; } 
                                                        ...
                                                        <td>
                                                            ...
                                                            ?>
                                                            <span class="badge bg-<?php echo $cls; ?>-subtle text-<?php echo $cls; ?> text-uppercase fs-11 px-2 py-1 mb-2 d-inline-block"><?php echo str_replace('_', ' ', $st); ?></span>
                                                            <div class="mt-1 text-muted" style="font-size: 11px;"><i class="ri-calendar-line me-1"></i><?php echo date('d M, Y', strtotime($lead['created_at'])); ?></div>
                                                            <div class="mt-2">
                                                                <button class="btn btn-xs btn-soft-info view-notes-btn w-100" data-bs-toggle="modal" data-bs-target="#notesModal" data-id="<?php echo $lead['id']; ?>" data-client="<?php echo htmlspecialchars($lead['client_name']); ?>"><i class="ri-chat-1-line me-1"></i> Notes</button>
                                                            </div>
                                                        </td>
                                                        <td class="text-center">
                                                            <div class="d-flex flex-column gap-2">
                                                                ...
                                                                <?php if ($user_role === 'admin'): ?>
                                                                    <a href="assigned_leads.php?action=delete&id=<?php echo $lead['id']; ?>" class="btn btn-sm btn-soft-danger" onclick="return confirm('Delete lead?');"><i class="ri-delete-bin-fill"></i></a>
                                                                <?php endif; ?>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr><td colspan="6" class="text-center text-muted py-4">No assigned leads found.</td></tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div class="card-footer border-top-0 py-3">
                                <div class="row align-items-center">
                                    <div class="col-md-4 mb-md-0">
                                        <form method="GET" action="assigned_leads.php" class="d-flex align-items-center justify-content-center justify-content-md-start gap-2">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (statusModal) {
        ...
        document.getElementById('statusForm').addEventListener('submit', function(e) {
            ...
            // 👇 Point to assigned_leads.php
            fetch('assigned_leads.php', { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
                    if(data.success) { 
    ...
    function loadNotes(leadId) {
        ...
        // 👇 Point to assigned_leads.php
        fetch(`assigned_leads.php?action=fetch_notes&lead_id=${leadId}`)
            .then(res => res.json())
            .then(data => {
                container.innerHTML = '';
                if(data.length > 0) {
    ...
    if (dateInput) {
        flatpickr(dateInput, {
            mode: "range", dateFormat: "Y-m-d", defaultDate: dateInput.value,
            onClose: function(selectedDates, dateStr, instance) {
                if (dateStr !== "" || dateInput.value !== "") { triggerFilterChange(); }
            }
        });
    }
    ...
    if (filterForm) {
        filterForm.addEventListener("change", function(e) {
            if (e.target.tagName === "SELECT" || e.target.tagName === "INPUT") {
                e.preventDefault(); triggerFilterChange();
            }
        });
    ...
    function triggerFilterChange() {
        ...
        if(limitSelect && !formData.has('limit')) { params.set('limit', limitSelect.value); }

        // 👇 POINT TO assigned_leads.php
        const url = "assigned_leads.php?" + params.toString();
        fetchData(url);
    }
    ...
    function fetchData(url) {
        ...
        fetch(url)
            ...
            .then(html => {
                ...
                // Update Badge
                if (badge && doc.querySelector(badgeID)) { badge.innerHTML = doc.querySelector(badgeID).innerHTML; }

                // Update Clear Button
                if (resetBtn && doc.querySelector(resetBtnID)) { resetBtn.innerHTML = doc.querySelector(resetBtnID).innerHTML; }
